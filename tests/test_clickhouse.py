    ...
    with Given("Tables t1, t2, t3 and MVs t1->t2, t1-t3 are created"):
        ...
        with When("Resume fetches for t2 at replica1"):
            ...
            with And("Second insert of the same block with 'deduplicate_blocks_in_dependent_materialized_views' setting should fail"):
                out = query_with_error(chi, "set deduplicate_blocks_in_dependent_materialized_views=1; insert into t1(a) values(5)", host=host0)
                assert "Quorum for previous write has not been satisfied yet" in out
            
        out = query_with_error(chi, "select t1.a t1_a, t2.a t2_a from t1 left outer join t2 using (a) order by t1_a settings join_use_nulls=1")
        print(out)
...
def test_ch_002():
    ...
    with And("Insert some data"):
        query(chi, "INSERT INTO test(team, user) values('team1', 'user1'),('team2', 'user2'),('team3', 'user3'),('team4', 'user4')")

...
def query(
    ...
    pod_names = kubectl.get_pod_names(k_node, chi_name, ns)
...
def drop_table_on_cluster(k_node, chi, cluster_name='all-sharded', table='default.test'):
    drop_local_sql = f'DROP TABLE {table} ON CLUSTER \'{cluster_name}\' SYNC'
    query(k_node, chi["metadata"]["name"], drop_local_sql, timeout=120)


def create_table_on_cluster(k_node, chi, cluster_name='all-sharded', table='default.test',
    ...
    query(k_node, chi["metadata"]["name"], create_local_sql, timeout=120)


def drop_distributed_table_on_cluster(k_node, chi, cluster_name='all-sharded', distr_table='default.test_distr', local_table='default.test'):
    drop_distr_sql = f'DROP TABLE {distr_table} ON CLUSTER \'{cluster_name}\''
    query(k_node, chi["metadata"]["name"], drop_distr_sql, timeout=120)
    drop_table_on_cluster(k_node, chi, cluster_name, local_table)


def create_distributed_table_on_cluster(k_node, chi, cluster_name='all-sharded', distr_table='default.test_distr', local_table='default.test',
    ...
    create_table_on_cluster(k_node, chi, cluster_name, local_table, fields_definition + ' ' + local_engine)
    create_distr_sql = f'CREATE TABLE {distr_table} ON CLUSTER \'{cluster_name}\' {fields_definition} {distr_engine}'
    query(k_node, chi["metadata"]["name"], create_distr_sql, timeout=120)

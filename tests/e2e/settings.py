...
def get_ch_version(test_file):
    ...
    return yaml.safe_load(open(os.path.join(current_dir, test_file), "r"))["spec"][
        "templates"
    ]["podTemplates"][0]["spec"]["containers"][0]["image"]


def get_docker_compose_path():
    caller_dir = os.path.dirname(
        os.path.abspath(inspect.currentframe().f_back.f_globals["__file__"])
    )
    docker_compose_project_dir = os.path.join(caller_dir, "../docker-compose")
    docker_compose_file_path = os.path.join(
        docker_compose_project_dir, "docker-compose.yml"
    )
...
test_namespace = (
    os.getenv("TEST_NAMESPACE") if "TEST_NAMESPACE" in os.environ else "test"
)
operator_version = (
    os.getenv("OPERATOR_VERSION")
    if "OPERATOR_VERSION" in os.environ
    else open(os.path.join(pathlib.Path(__file__).parent.absolute(), "../../release"))
    .read(1024)
    .strip(" \r\n\t")
)
operator_namespace = (
...
operator_install = (
    os.getenv("OPERATOR_INSTALL") if "OPERATOR_INSTALL" in os.environ else "yes"
)
minio_namespace = (
    os.getenv("MINIO_NAMESPACE") if "MINIO_NAMESPACE" in os.environ else "minio"
)
operator_docker_repo = (
    os.getenv("OPERATOR_DOCKER_REPO")
...
image_pull_policy = (
    os.getenv("IMAGE_PULL_POLICY") if "IMAGE_PULL_POLICY" in os.environ else "Always"
)

...
def launch(command, ok_to_fail=False, ns=namespace, timeout=6000):
    # Build command
    cmd = f"{kubectl_cmd} "
...
def delete_chi(chi, ns=namespace, wait = True, ok_to_fail=False):
    with When(f"Delete chi {chi}"):
        launch(f"delete chi {chi}", ns=ns, timeout=6000, ok_to_fail=ok_to_fail)
        if wait:
            wait_objects(chi,
                {
                    "statefulset": 0,
...
def apply(config, ns=namespace, validate=True, timeout=120):
    with When(f"{config} is applied"):
        launch(f"apply --validate={validate} -f {config}", ns=ns, timeout=timeout)


def delete(config, ns=namespace, timeout=120):

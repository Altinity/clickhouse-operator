...
def test_031(self):
    ...
    with When("I apply templates with annotations and chis"):
        kubectl.apply(manifest=util.get_full_path("manifests/chit/tpl-test-031.yaml", False), ns=settings.test_namespace)
        time.sleep(5)
        kubectl.apply(ns=settings.test_namespace, manifest=util.get_full_path("manifests/chi/test-031-from-tpl.yaml", False))
    ...
    with Then("I check only allowed annotations are propagated"):
        while not (len(kubectl.get_pod_names(chi_name='test-031-from-tpl')) and len(kubectl.get_pod_names(chi_name='test-031-from-tpl'))):
            time.sleep(1)

        with By("I check propagation from template"):
            chi_from_tpl_name = kubectl.get_pod_names(chi_name='test-031-from-tpl')[0]
            annotations = kubectl.launch(command=f"get pod -n test {chi_from_tpl_name} -o jsonpath='{{.metadata.annotations}}'")
            assert "incl" in annotations, error()
            assert "excl" not in annotations, error()

        with And("I check propagation from chi to StatefulSets, ConfigMaps, PVCs"):
            ...
            for cm in configmaps_wo_tpl_name:
                annotations = kubectl.launch(command=f"get statefulset -n test {cm} -o jsonpath='{{.metadata.annotations}}'")
                assert "incl" in annotations, error()
                assert "excl" not in annotations, error()

            pvcs_wo_tpl_name = kubectl.get_obj_names(chi_name='test-031-wo-tpl', obj_type="pvc")
            for pvc in pvcs_wo_tpl_name:
                annotations = kubectl.launch(command=f"get statefulset -n test {pvc} -o jsonpath='{{.metadata.annotations}}'")
                assert "incl" in annotations, error()
                assert "excl" not in annotations, error()


    with Finally("I restore original operator state"):
        kubectl.delete_chi('test-031-from-tpl')

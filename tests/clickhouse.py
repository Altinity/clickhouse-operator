...
def query(chi_name, sql, with_error=False, host="127.0.0.1", port="9000", user="default", pwd="", ns=settings.test_namespace, timeout=60):
    ...
    if with_error:
        return kubectl.kubectl(f"exec {pod_name} -n {ns} -- clickhouse-client -mn -h {host} --port={port} -u {user} {pwd_str} --query=\"{sql}\" 2>&1", ok_to_fail=True, timeout=timeout)
    else:
        return kubectl.kubectl(f"exec {pod_name} -n {ns} -- clickhouse-client -mn -h {host} --port={port} -u {user} {pwd_str} --query=\"{sql}\"", timeout=timeout)

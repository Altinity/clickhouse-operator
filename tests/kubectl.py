...
def kube_wait_object(_type, name, label="", count=1, ns=namespace, retries=max_retries):
    with Then(f"{count} {_type}(s) {name} should be created"):
        for i in range(1, retries):
            counts = kube_get_count(_type, ns=ns, name=name, label=label)
            if counts >= count:
                break
            with Then("Not ready. Wait for " + str(i*5) + " seconds"):
                time.sleep(i*5)
...
def kube_wait_field(_object, name, field, value, ns=namespace, retries=max_retries):
...
def kube_wait_jsonpath(_object, name, field, value, ns=namespace, retries=max_retries):
...
def kube_get_field(_object, name, field, ns=namespace):
...
def kube_get_pod_volumes(chi_name, ns=namespace):
    volume_mounts = kube_get_pod_spec(chi_name, ns)["containers"][0]["volumeMounts"]

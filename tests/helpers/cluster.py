...
class Node(object):
    ...
    def restart(self, timeout=5000, retries=5, safe=True):
        """Restart node.
        """
        self.close_bashes()
    ...
    def start(self, timeout=5000, retries=5):
        """Start node.
        """
        for retry in range(retries):
    ...
    def stop(self, timeout=5000, retries=5, safe=True):
        """Stop node.
        """
        self.close_bashes()
    ...
    def cmd(self, cmd, message=None, exitcode=None, steps=True, shell_command="bash --noediting", no_checks=False,
            raise_on_exception=False, step=By, timeout=6000, *args, **kwargs):
...
class Cluster(object):
    ...
    def bash(self, node, timeout=6000, command="bash --noediting"):
            ...
            for attempt in range(max_attempts):
                with When(f"attempt {attempt}/{max_attempts}"):
                    with By("checking if images already pulled"):
                        cmd = self.command(None, "docker images | grep registry.gitlab.com/altinity-qa/clickhouse/cicd/clickhouse-operator", exitcode=None, timeout=timeout)
                        if cmd.exitcode != 0:
                    ...
                    with And("executing docker-compose down just in case it is up"):
                        cmd = self.command(None, f"{self.docker_compose} down -v --remove-orphans 2>&1 | tee", exitcode=None, timeout=timeout)
                        if cmd.exitcode != 0:
                            continue
    ...
    def command(self, node, command, message=None, exitcode=None, steps=True, bash=None, timeout=6000, *args, **kwargs):
        """Execute and check command.
        :param node: name of the service

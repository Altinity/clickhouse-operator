...
function wait_minio_to_start() {
    ...
    while [[ $(kubectl --namespace="${namespace}" get pods | grep "${pod_name}" | grep "Running" | wc -l) == "0" ]]; do
        printf "."
        sleep 1
    done
    echo "...DONE"
    ...
    while [[ $(kubectl --namespace="${namespace}" get deployments | grep "${deployment_name}" | grep "1/1" | wc -l) == "0" ]]; do
        printf "."
        sleep 1
    done
    echo "...DONE"
...
function wait_minio_bucket() {
    ...
    # Check minio tenatna have all pods ready
    while [[ $(kubectl --namespace="${namespace}" get jobs | grep "${job_name}" | grep "1/1" | wc -l) == "0" ]]; do
        printf "."
        sleep 1
    done
    echo "...DONE"

...
// chopGeneratedObjectsLabelSelector builds a label selector that matches objects generated by the operator.
// This is the server-side counterpart of the client-side Controller.isTrackedObject() check, which requires
// Labeler.IsCHOPGeneratedObject(). Both are built out of the same labeler, so that the two can not diverge.
func chopGeneratedObjectsLabelSelector() string {
	labeler := chiLabeler.New(nil)
	return k8sLabels.SelectorFromSet(k8sLabels.Set{
		labeler.Get(commonLabeler.LabelAppName): labeler.Get(commonLabeler.LabelAppValue),
	}).String()
}
...
func initClickHouse(ctx context.Context) {
	...
	// Create Informers
	// Kube objects are filtered server-side down to operator-generated objects only. GetInformerNamespace()
	// falls back to all namespaces as soon as watched namespaces are specified as anything other than a single
	// literal namespace name, so without this selector the informer caches - and the DeltaFIFO feeding them -
	// scale with the size of the whole cluster instead of with the number of managed installations.
	// CHOp custom resource informers below are intentionally left unfiltered.
	kubeObjectsLabelSelector := chopGeneratedObjectsLabelSelector()
	log.V(1).F().Info("Kube informers label selector: '%s'", kubeObjectsLabelSelector)
	kubeInformerFactory := kubeinformers.NewSharedInformerFactoryWithOptions(
		...
		kubeinformers.WithTweakListOptions(func(options *meta.ListOptions) {
			options.LabelSelector = kubeObjectsLabelSelector
		}),
	)

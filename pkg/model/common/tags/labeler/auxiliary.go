...
//
// Reads the same label set CHOPGeneratedSelector hands to the server-side informer filter, so the
// two agree by construction rather than by convention: a narrowed List/Watch cannot drop an object
// this would have accepted.
//
// That guarantee costs a little more per event than the two direct map lookups it replaced, but
// no allocation: the map does not escape. Comparing the set directly rather than routing it
// through a k8s Selector is what keeps it allocation-free - SelectorFromSet builds a requirement
// slice per call, for a single equality check.
func (l *Labeler) IsCHOPGeneratedObject(meta meta.Object) bool {
	labels := meta.GetLabels()
	for key, value := range l.CHOPGeneratedSelector() {
		if labels[key] != value {
			return false
		}
	}

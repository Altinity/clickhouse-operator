...
func (n *Normalizer) FillCHIAddressInfo() {
	...
	n.chi.WalkPodTemplates(func(template *chiv1.ChiPodTemplate) {
		for i := range template.PodDistribution {
			podDistribution := &template.PodDistribution[i]
			if podDistribution.Type == chiv1.PodDistributionMaxNumberPerNode {
	...
	if requestedClusterScopeCyclesNum == 1 {
		...
		clusterScopeCycleSize = int(math.Ceil(float64(n.chi.HostsCount()) / float64(requestedClusterScopeCyclesNum)))
	}

	n.chi.WalkHostsFullPath(chiScopeCycleSize, clusterScopeCycleSize, func(
		chi *chiv1.ClickHouseInstallation,

...
// getRemoteServersClusterSecret renders the cluster-level <secret> element
// inside a remote_servers cluster block. Plaintext values are XML-escaped to
// keep remote_servers.xml well-formed when a user-supplied secret happens to
// contain '<', '>', '&', or quote characters; SecretRef/Auto reference the
// env var injected into the host's environment by the secret-mounting code.
// Nil-safe via ClusterSecret.Source() returning Unspecified on nil receivers.
// Signature order (secret, indent, b) mirrors the file's peer helpers
// (e.g. getRemoteServersReplica) where the domain object is first and the
// output buffer is last.
func (c *Generator) getRemoteServersClusterSecret(secret *chi.ClusterSecret, indent int, b *bytes.Buffer) {
	switch secret.Source() {
	case chi.ClusterSecretSourcePlaintext:
		var escaped bytes.Buffer
		if err := xml.EscapeText(&escaped, []byte(secret.Value)); err != nil {
			log.Error("FAILED to XML-escape cluster secret value: %v", err)
			return
		}
		util.Iline(b, indent, "<secret>%s</secret>", escaped.String())
...
func (c *Generator) getRemoteServerClusters(selector *config.HostSelector, indent int) *bytes.Buffer {
	...
	c.cr.WalkClusters(func(cluster chi.ICluster) error {
		...
		// <secret>VALUE</secret>
		c.getRemoteServersClusterSecret(cluster.GetSecret(), indent+4, b)
...
func (c *Generator) getRemoteClustersAutogenerated(selector *config.HostSelector, indent int) *bytes.Buffer {
	...
	// Autogenerated clusters (all-replicated / all-sharded / all-clusters)
	// share the first user-defined cluster's secret. Mixed-secret multi-cluster
	// CHIs are unsupported here; the normalizer emits a warning when cluster
	// secret sources disagree.
	firstCluster := c.cr.FindCluster(0)
	...
	c.getRemoteServersClusterSecret(firstCluster.GetSecret(), indent+4, b)

...
func (c *Controller) addEventHandlers(
	...
	chopInformerFactory.Clickhouse().V1().ClickHouseInstallations().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("chiInformer.AddFunc - %s/%s added", chi.Namespace, chi.Name)
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.V(2).Info("chiInformer.DeleteFunc - CHI %s/%s deleted", chi.Namespace, chi.Name)
	...
	chopInformerFactory.Clickhouse().V1().ClickHouseInstallationTemplates().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("chitInformer.AddFunc - %s/%s added", chit.Namespace, chit.Name)
		...
		UpdateFunc: func(old, new interface{}) {
			...
			log.V(2).Info("chitInformer.UpdateFunc - %s/%s", newChit.Namespace, newChit.Name)
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.V(2).Info("chitInformer.DeleteFunc - %s/%s deleted", chit.Namespace, chit.Name)
	...
	chopInformerFactory.Clickhouse().V1().ClickHouseOperatorConfigurations().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("chopInformer.AddFunc - %s/%s added", chopConfig.Namespace, chopConfig.Name)
		...
		UpdateFunc: func(old, new interface{}) {
			...
			log.V(2).Info("chopInformer.UpdateFunc - %s/%s", newChopConfig.Namespace, newChopConfig.Name)
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.V(2).Info("chopInformer.DeleteFunc - %s/%s deleted", chopConfig.Namespace, chopConfig.Name)
	...
	kubeInformerFactory.Core().V1().Services().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("serviceInformer AddFunc %s/%s", service.Namespace, service.Name)
		},
		UpdateFunc: func(old, new interface{}) {
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.V(2).Info("serviceInformer DeleteFunc %s/%s", service.Namespace, service.Name)
		},
	})
	...
	kubeInformerFactory.Core().V1().Endpoints().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("endpointsInformer AddFunc %s/%s", endpoints.Namespace, endpoints.Name)
		},
		UpdateFunc: func(old, new interface{}) {
			...
			if equal {
				log.V(2).Info("onUpdateEndpoints(%s/%s): no changes found", oldEndpoints.Namespace, oldEndpoints.Name)
				// No need to react
				return
			}
			...
			for path := range diff.Added {
				log.V(2).Info("onUpdateEndpoints(%s/%s): added %v", oldEndpoints.Namespace, oldEndpoints.Name, path)
			...
			for path := range diff.Removed {
				log.V(2).Info("onUpdateEndpoints(%s/%s): removed %v", oldEndpoints.Namespace, oldEndpoints.Name, path)
			}
			for path := range diff.Modified {
				log.V(2).Info("onUpdateEndpoints(%s/%s): modified %v", oldEndpoints.Namespace, oldEndpoints.Name, path)
			}

			if added {
				log.V(1).Info("endpointsInformer UpdateFunc(%s/%s) IP ASSIGNED %v", newEndpoints.Namespace, newEndpoints.Name, newEndpoints.Subsets)
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.V(2).Info("endpointsInformer DeleteFunc %s/%s", endpoints.Namespace, endpoints.Name)
		},
	})
	...
	kubeInformerFactory.Apps().V1().StatefulSets().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("statefulSetInformer AddFunc %s/%s", statefulSet.Namespace, statefulSet.Name)
		...
		UpdateFunc: func(old, new interface{}) {
			...
			log.V(2).Info("statefulSetInformer UpdateFunc %s/%s", statefulSet.Namespace, statefulSet.Name)
		},
		DeleteFunc: func(obj interface{}) {
			statefulSet := obj.(*apps.StatefulSet)
			if !c.isTrackedObject(&statefulSet.ObjectMeta) {
			...
			log.V(2).Info("statefulSetInformer DeleteFunc %s/%s", statefulSet.Namespace, statefulSet.Name)
			//controller.handleObject(obj)
		},
	})
	...
	kubeInformerFactory.Core().V1().Pods().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.V(2).Info("podInformer AddFunc %s/%s", pod.Namespace, pod.Name)
		},
		UpdateFunc: func(old, new interface{}) {
			pod := old.(*core.Pod)
			if !c.isTrackedObject(&pod.ObjectMeta) {
			...
			log.V(2).Info("podInformer UpdateFunc %s/%s", pod.Namespace, pod.Name)
		},
		DeleteFunc: func(obj interface{}) {
			pod := obj.(*core.Pod)
			if !c.isTrackedObject(&pod.ObjectMeta) {
			...
			log.V(2).Info("podInformer DeleteFunc %s/%s", pod.Namespace, pod.Name)
		},
	})
}
...
func (c *Controller) Run(ctx context.Context) {
	...
	log.V(1).Info("ClickHouseInstallation controller: starting workers number: %d", workersNum)
	for i := 0; i < workersNum; i++ {
		log.V(1).Info("ClickHouseInstallation controller: starting worker %d out of %d", i+1, workersNum)
...
func (c *Controller) updateWatchAsync(namespace, name string, hostnames []string) {
	if err := metrics.InformMetricsExporterAboutWatchedCHI(namespace, name, hostnames); err != nil {
		log.V(1).Info("FAIL update watch (%s/%s): %q", namespace, name, err)
	} else {
...
func (c *Controller) deleteWatchAsync(namespace, name string) {
	if err := metrics.InformMetricsExporterToDeleteWatchedCHI(namespace, name); err != nil {
		log.V(1).Info("FAIL delete watch (%s/%s): %q", namespace, name, err)
	} else {
...
func (c *Controller) addChit(chit *chi.ClickHouseInstallationTemplate) error {
	log.V(1).Info("addChit(%s/%s)", chit.Namespace, chit.Name)
...
func (c *Controller) updateChit(old, new *chi.ClickHouseInstallationTemplate) error {
	if old.ObjectMeta.ResourceVersion == new.ObjectMeta.ResourceVersion {
		log.V(2).Info("updateChit(%s/%s): ResourceVersion did not change: %s", old.Namespace, old.Name, old.ObjectMeta.ResourceVersion)
		// No need to react
		return nil
	}

	log.V(2).Info("updateChit(%s/%s):", new.Namespace, new.Name)
...
func (c *Controller) deleteChit(chit *chi.ClickHouseInstallationTemplate) error {
	log.V(2).Info("deleteChit(%s/%s):", chit.Namespace, chit.Name)
...
func (c *Controller) addChopConfig(chopConfig *chi.ClickHouseOperatorConfiguration) error {
	if c.chop.ConfigManager.IsConfigListed(chopConfig) {
		log.V(1).Info("addChopConfig(%s/%s) already known config - do nothing", chopConfig.Namespace, chopConfig.Name)
	} else {
		log.V(1).Info("addChopConfig(%s/%s) new, previously unknown config, need to apply", chopConfig.Namespace, chopConfig.Name)
		// TODO
		// NEED REFACTORING
		// os.Exit(0)
	}
...
func (c *Controller) updateChopConfig(old, new *chi.ClickHouseOperatorConfiguration) error {
	if old.ObjectMeta.ResourceVersion == new.ObjectMeta.ResourceVersion {
		log.V(2).Info("updateChopConfig(%s/%s): ResourceVersion did not change: %s", old.Namespace, old.Name, old.ObjectMeta.ResourceVersion)
		// No need to react
		return nil
	}

	log.V(2).Info("updateChopConfig(%s/%s):", new.Namespace, new.Name)
	// TODO
	// NEED REFACTORING
	//os.Exit(0)
...
func (c *Controller) deleteChopConfig(chopConfig *chi.ClickHouseOperatorConfiguration) error {
	log.V(2).Info("deleteChopConfig(%s/%s):", chopConfig.Namespace, chopConfig.Name)
	// TODO
	// NEED REFACTORING
	//os.Exit(0)
...
func (c *Controller) updateCHIObject(chi *chi.ClickHouseInstallation) error {
	...
	if err != nil {
		// Error update
		log.V(1).Info("ERROR update CHI (%s/%s): %q", namespace, name, err)
		return err
	}

	if chi.ObjectMeta.ResourceVersion != new.ObjectMeta.ResourceVersion {
		// Updated
		log.V(2).Info("updateCHIObject(%s/%s): ResourceVersion bump %s=>%s",
...
func (c *Controller) updateCHIObjectStatus(chi *chi.ClickHouseInstallation, tolerateAbsence bool) error {
	namespace, name := util.NamespaceName(chi.ObjectMeta)
	log.V(2).Info("Update CHI status (%s/%s)", namespace, name)
	...
	if err != nil {
		...
		log.V(1).Info("ERROR GetCHI (%s/%s): %q", namespace, name, err)
		return err
	}
	if cur == nil {
		if tolerateAbsence {
		...
		log.V(1).Info("ERROR GetCHI (%s/%s): NULL returned", namespace, name)
...
func waitForCacheSync(name string, stopCh <-chan struct{}, cacheSyncs ...cache.InformerSynced) bool {
	log.V(1).Info("Syncing caches for %s controller", name)
	if !cache.WaitForCacheSync(stopCh, cacheSyncs...) {
		utilruntime.HandleError(fmt.Errorf(messageUnableToSync, name))
		return false
	}
	log.V(1).Info("Caches are synced for %s controller", name)
	return true
}

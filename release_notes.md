...
* **Bumped the Go toolchain to address stdlib CVEs in the operator and metrics-exporter images.** The `go` directive in `go.mod` — the single source the Dockerfiles and CI derive `GO_VERSION` from — is bumped `1.26.5` → `1.26.8`. No API or behavior changes.

  `govulncheck` reports five standard-library vulnerabilities reachable from operator/exporter code on a 1.26.5 build, and none on 1.26.8:
  ...
  * **GO-2026-5026** — `golang.org/x/net/idna` as bundled into the standard library.
  ...
  All five are remediated in 1.26.6; 1.26.7 and 1.26.8 add no further advisories. 1.26.7 repairs an unencrypted-HTTP/2 regression that the 1.26.6 fix for GO-2026-6089 introduced in `net/http`, and 1.26.8 carries cgo, compiler, runtime, `debug/elf` and `os` fixes. Neither is reachable here — the operator never sets `ReadHeaderTimeout`, and the images build `CGO_ENABLED=0` for `linux/amd64` and `linux/arm64` — so taking them simply keeps the shipped images on the current 1.26 patch.

  The FIPS module is unaffected: `GOFIPS140=v1.0.0` selects a frozen snapshot, and both `lib/fips140/v1.0.0-c2097c7c.zip` and the `fips140.sum` that pins it are byte-identical between the 1.26.5 and 1.26.8 toolchains, so FIPS evidence and ACVP results do not change.
...
### Behavior Changes
* **A pod that will not terminate is now force-deleted so a StatefulSet recreate finishes in one pass** ([PR #2080](https://github.com/Altinity/clickhouse-operator/pull/2080), [#2078](https://github.com/Altinity/clickhouse-operator/issues/2078)). Recreating a host's StatefulSet scales it to 0 and waits for the pod to go. When the pod would not terminate — a ClickHouse wedged on Keeper hangs on shutdown — the wait spent its whole `reconcile.statefulSet.update.timeout` budget, the result was discarded, and the pass aborted with the host left at `Replicas=0` and no pod. On a large installation the next pass might not return to that host for a long time, so a shard could be left serving on one replica, and the same path serves Keeper, so an ensemble could drop below Raft quorum.

  The operator now deletes that pod with grace period 0 — a SIGKILL to ClickHouse — and finishes Delete and Create in the same pass. It emits a `HostPodForceDeleted` warning Event against the CR, so the decision is visible in `kubectl describe`. **Only a fully spent wait budget triggers it**: a transient API error during the wait returns in milliseconds and never forces. The escalation is best-effort — if the pod delete itself fails, the StatefulSet delete still runs, as it did before.

  Two related fixes ship with it. A host stranded at `Replicas=0` by an earlier failed pass now re-enters the same escalation instead of skipping the wait entirely, so it can recover rather than retrying the identical failure. And a pod that is terminating no longer counts as its shard's healthy peer: deleting a pod only stamps `deletionTimestamp`, and kubelet keeps the readiness probe running, so a host wedged in shutdown kept reporting Ready and the operator would disrupt its last serving sibling.

* **ZooKeeper endpoint changes no longer restart ClickHouse** ([PR #2074](https://github.com/Altinity/clickhouse-operator/pull/2074)). `configurationRestartPolicy` has advertised `zookeeper/*: "no"` since 0.25.6, but the operator ignored it and restarted every host on any change to `spec.configuration.zookeeper`. The generated ZooKeeper config is written to the per-host `conf.d` directory, which ClickHouse re-reads without a process restart, so the restart was never needed for an endpoint edit. The policy is now honored.

  This supersedes the 0.27.2 note below about a one-time rolling restart for CHI→CHK keeper references: that restart no longer happens. If you upgraded through 0.27.2 or 0.27.3 with a `keeper:` reference in place, you have already taken that restart once; coming from 0.27.1 or earlier you will not take it at all.

  Two carve-outs are deliberate. Turning ZooKeeper on or off still restarts regardless of the policy, because that adds or removes the `<distributed_ddl>` section along with `<zookeeper>` and ClickHouse initializes its DDL worker at startup. And a change between two configurations that both amount to "no ZooKeeper" never restarts, since neither generates any config. Setting `zookeeper/*: "yes"` restores a restart on ordinary endpoint edits: rule sets from a `ClickHouseOperatorConfiguration` are appended to the operator's own, and the last matching rule wins, so your entry overrides the shipped one.

  **If you resize or migrate a ZooKeeper ensemble, do not decommission the old servers the moment the CHI reports `Completed`.** Without the restart the new endpoint list reaches ClickHouse only via kubelet's ConfigMap sync and ClickHouse's own reload, which can trail the status by up to a minute; the operator waits only `reconcile.configMapPropagationTimeout`, whose 10s default applies solely when `.spec.reconciling` is absent altogether — set any field under it and the wait becomes zero. Raise that timeout, or confirm `system.zookeeper_connection` on each host before retiring the old ensemble.

  Affects any CHI whose effective cluster ZooKeeper settings change — `spec.configuration.zookeeper`, a per-cluster `spec.configuration.clusters[].zookeeper`, or a `keeper:` reference. The reference case is the one to watch: resolved Keeper endpoints are appended into that same field, so with the default `serviceType: Replicas` — which enumerates one client Service per Keeper host — scaling a CHK changes the CHI's node list and no longer restarts ClickHouse. Under `serviceType: Service` the single Service name is stable, so scaling changes nothing either way.
...
## Release 0.27.3
### New Features
* **Helm `serviceMonitor.enabled` now covers Keeper metrics** ([#2038](https://github.com/Altinity/clickhouse-operator/issues/2038)). The chart shipped a single `ServiceMonitor` scoped to the operator's own metrics endpoint, so enabling it left `ClickHouseKeeper` metrics unscraped — Keeper had to be wired into Prometheus by hand. The chart now also emits a `ServiceMonitor` for Keeper metrics. Helm installs only; no change for users who leave `serviceMonitor.enabled: false`.

### Behavior Changes
* **Unhealthy replicas in a shard are now reconciled before healthy ones** ([#1704](https://github.com/Altinity/clickhouse-operator/issues/1704)). Previously the operator reconciled a shard's hosts in fixed order, so if the operator restarted midway through a shard it could take down a healthy replica while a sibling was still down, leaving the shard with no serving replica. Hosts that need recovery are now reconciled first, and a host whose shard has no other healthy replica has its restart deferred rather than proceeding — the shard keeps serving and the reconcile retries once a peer is back. The deferral is scoped so that a shard waiting on a peer no longer blocks progress on its sibling shards. Reconcile ordering within a shard is therefore no longer stable across runs; nothing else about the resulting state changes.
* **Cluster-scoped reconcile hooks skip unreachable hosts instead of aborting the reconcile.** A cluster-scoped hook targeting every host previously failed the whole reconcile when any single host could not serve SQL — during a scale-up, a host that does not exist yet. Such hosts are now skipped with a `HookSkippedUnreachableHost` event and the hook still runs on the hosts it can reach; the reconcile only fails if *no* target host was reachable. Hook execution also now collects failures across all shards rather than stopping at the first, so a `failurePolicy: Fail` decision sees the full blast radius.

### Fixed
* **Operator no longer panics and crash-loops on watch reconnect** ([#2050](https://github.com/Altinity/clickhouse-operator/issues/2050), [#1882](https://github.com/Altinity/clickhouse-operator/issues/1882)). When a watch connection dropped, client-go re-listed and delivered any object deleted while the connection was down as a `cache.DeletedFinalStateUnknown` tombstone rather than the typed object. Every informer `DeleteFunc` type-asserted the payload directly, so the tombstone panicked the handler goroutine — and because client-go re-panics by default, the whole operator process died and restarted. All delete handlers now unwrap the tombstone and drop payloads that cannot be decoded. The metrics-exporter's config watcher is fixed the same way: it previously exited the container on *any* delete event, including a relist tombstone, which caused spurious sidecar restarts.
* **`reconcile.host.drop.replicas.onLostVolume: "no"` now actually prevents `SYSTEM DROP REPLICA` on volume loss.** The setting was never consulted, so a host that lost its storage volume had its replica dropped from ZooKeeper/Keeper regardless — the opt-out silently did nothing, and only `reconcile.statefulSet.recreate.onDataLoss: abort` offered any protection. The knob is now honored. **If you were relying on `onLostVolume: "no"` for data safety, it takes effect for the first time on upgrade**; conversely, if you had it set to `"no"` without intending to block replica cleanup, lost-volume hosts will now retain their replica entries.
* **Reconcile SQL hook queries now fire on every host, not just the first** ([#2052](https://github.com/Altinity/clickhouse-operator/issues/2052)). Hook execution mutated the hook's own query list in place while substituting per-host values, so after the first host the stored queries were left blanked and every subsequent host ran an empty hook — silently, with no error. The substitution now operates on a copy.
* **Image-only upgrades again exclude the host before restarting it** ([#2055](https://github.com/Altinity/clickhouse-operator/issues/2055)). A change limited to the container image skipped host exclusion, so the pod was restarted without being drained from the load balancer or deprioritized in `remote_servers` — in-flight queries were dropped and distributed queries were routed to a restarting host. Image changes now take the normal exclude/restart/include path.
* **Reconcile is triggered again when the operator's own pod IP changes.** The operator's IP appears in the generated ClickHouse `networks/ip` and `host_regexp` settings; after an operator pod restart on a new IP, those settings were left stale until an unrelated reconcile happened to refresh them.
* **Transient Kubernetes API errors are retried instead of failing the reconcile.** Read paths in both the `ClickHouseInstallation` and `ClickHouseKeeper` kube drivers now retry on transient API failures (timeouts, connection resets, `TooManyRequests`) through a shared retry wrapper, rather than surfacing a momentary API-server blip as a reconcile error.

### Documentation
* **Added an exhaustive operator-configuration example.** `docs/chi-examples/99-clickhouseoperatorconfiguration-max.yaml` documents every option the `ClickHouseOperatorConfiguration` CRD declares, as the counterpart of `99-clickhouseinstallation-max.yaml`. `70-chop-config.yaml` remains the short, deployable starting point and the sample shipped in the OperatorHub bundle. The new file also documents the merge semantics that are easy to get wrong: explicitly-set values pin over future defaults, and list-valued settings append to the operator's list rather than replacing it.
* **Added third-party notices and clarified project license attribution** ([PR #2061](https://github.com/Altinity/clickhouse-operator/pull/2061)).

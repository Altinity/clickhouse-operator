...
* **Operator informers and caches no longer hold every object in the cluster.** They are narrowed at the API server to `clickhouse.altinity.com/app=chop`, and the Keeper manager caches to `clickhouse-keeper.altinity.com/app=chop`, so memory scales with the number of managed installations rather than cluster size. Applies whenever more than a single namespace is watched. Removing an operator object's app label by hand hides it from the watch; nothing is deleted. by [@sunsingerus](https://github.com/sunsingerus). Approach proposed by [@gregakinman](https://github.com/gregakinman) in [#2058](https://github.com/Altinity/clickhouse-operator/pull/2058)

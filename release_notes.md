...
* **Operator informers and caches no longer hold every object in the cluster.** Kubernetes informers are narrowed at the API server to `clickhouse.altinity.com/app=chop`, and the Keeper manager caches to `clickhouse-keeper.altinity.com/app=chop`, so what they hold scales with the number of managed installations rather than with cluster size. Applies whenever more than a single named namespace is watched — the shipped `kube-system` install, OLM AllNamespaces and MultiNamespace, or any multi-entry or regexp `include`; a Helm install in its own namespace is already scoped and unchanged. An operator-generated object whose app label is removed by hand becomes invisible to the watch rather than merely ignored — nothing is deleted or restarted, and re-applying the label restores it. Both effective selectors are logged at startup; RBAC is unchanged. by [@sunsingerus](https://github.com/sunsingerus). Approach proposed by [@gregakinman](https://github.com/gregakinman) in [#2058](https://github.com/Altinity/clickhouse-operator/pull/2058)

* **Keeper initialization is retried instead of attempted once.** Narrowing the Keeper cache makes controller-runtime resolve REST mappings while the manager is built, so start-up now contacts the API server where it previously did not, and a single failure used to disable Keeper reconciliation for the lifetime of the process. It is retried indefinitely without exiting, and ClickHouse reconciliation never waits on it. A missing `ClickHouseKeeperInstallation` CRD is not such a failure. by [@sunsingerus](https://github.com/sunsingerus)
...
* **A StatefulSet the operator cannot read is no longer rebuilt.** Only `NotFound` means the StatefulSet is absent; any other read error — RBAC still propagating, an overloaded API server — escalated to a recreate that deleted the StatefulSet and its pods as soon as a later read succeeded. Such a read now raises a host reconcile error and retries on the next pass. Affects ClickHouse and Keeper alike. by [@sunsingerus](https://github.com/sunsingerus)

* **Keeper no longer reports phantom data loss on a transient read.** Keeper object reads went through the manager cache, where a miss is indistinguishable from absence, so an unreadable `PersistentVolumeClaim` could trigger lost-volume handling and a StatefulSet recreate. By-name reads now bypass the cache. by [@sunsingerus](https://github.com/sunsingerus)
